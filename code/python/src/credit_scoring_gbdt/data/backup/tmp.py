...
API_KEY = "c055ec4fdf611f8633aacafe4a82d3df411f9ba9"
INPUT_XLSX = "inn.xlsx"
OUTPUT_JSONL = "companies.jsonl"
...
def fetch_and_filter_company_data(inn: str, api_key: str):
    ...
    def extract_financial_history():
        ...
        for entry in raw_list:
            ...
            tax_map = {"usn": 0.0, "pension": 0.0, "medical": 0.0, "social": 0.0, "penalty": 0.0, "other": 0.0}
...
def run_for_excel_inns():
    df = pd.read_excel(INPUT_XLSX)
    inns = df["INN"].dropna().astype(str).tolist()[:100]

    with open(OUTPUT_JSONL, "a", encoding="utf-8") as out_file:
        for inn in inns:
            print(f"📡 Fetching {inn} ...")
            try:
                data = fetch_and_filter_company_data(inn, API_KEY)
                if data:
                    out_file.write(json.dumps(data, ensure_ascii=False) + "\n")
            except Exception as e:
                print(f"❌ Ошибка при обработке {inn}: {e}")
            sleep(0.5)
